...
pub fn render(frame: &mut Frame, area: Rect, state: &AppState) {
    let (search_style, impact_style, context_style, memory_style) = tab_styles(&state.mode);
...
fn tab_styles(mode: &ActiveMode) -> (Style, Style, Style, Style) {
    ...
    match mode {
        ActiveMode::Search => (active, inactive, inactive, inactive),
        ActiveMode::Impact => (inactive, active, inactive, inactive),
        ActiveMode::Context => (inactive, inactive, active, inactive),
        ActiveMode::Memory => (inactive, inactive, inactive, active),
    }
}
