...
///
/// All four query types (search, impact, context, snippet) are memoised here
/// for the lifetime of a `codesearch tui` session.  A cache hit means no
/// async task is spawned and results appear instantly, which matters because
/// the underlying use cases (ONNX embedding, DuckDB BFS) can take hundreds of
