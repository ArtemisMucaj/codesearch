//! Installing and removing the agent integrations.
//!
...
//!
//! The live nudge itself is decided by [`crate::agent::pre_tool_call`], invoked
//! across platforms via the `codesearch pre-tool-call` command.
...
pub fn dispatch(command: &Commands) -> Result<String> {
    match command {
        ...
        _ => unreachable!("install::dispatch called with a non-install command"),
    }
}
