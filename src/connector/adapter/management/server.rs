...
async fn index(State(_state): State<AppState>) -> Json<Value> {
    Json(json!({
        ...
        "endpoints": [
            ...
            { "method": "GET", "path": "/api/clusters", "description": "file-dependency Leiden clusters (?global= for one namespace-wide run across every repository)" },
            { "method": "GET", "path": "/api/symbol-clusters", "description": "symbol call-graph communities" },
            { "method": "GET", "path": "/api/graph", "description": "render-ready community graph with edges (?level=file|symbol&aggregate=&global=)" },
