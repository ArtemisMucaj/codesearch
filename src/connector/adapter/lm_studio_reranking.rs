...
/// LLM-based reranker that delegates to a [`ChatClient`] (e.g. LM Studio via the
...
///
/// The chat client and model are controlled by the same environment variables used
...
impl LmStudioReranking {
    pub fn new(client: Arc<dyn ChatClient>) -> Self {
        let model_name = std::env::var("ANTHROPIC_MODEL")
            .unwrap_or_else(|_| "lm-studio-reranker".to_string());
    ...
    fn parse_scores(text: &str, expected: usize) -> Option<Vec<f32>> {
        ...
        if scores.len() != expected {
            warn!(
                "LmStudioReranking: expected {} scores, got {}",
                expected,
                scores.len()
            );
            return None;
...
#[async_trait]
impl RerankingService for LmStudioReranking {
    async fn rerank(
        &self,
        query: &str,
        ...
        debug!(
            "LmStudioReranking: reranking {} results for query: {}",
            results.len(),
            query
        );
        ...
        let scores = match self.client.complete(SYSTEM_PROMPT, &user_prompt).await {
            Ok(text) => {
                debug!("LmStudioReranking raw response: {text}");
                Self::parse_scores(&text, n).unwrap_or_else(|| {
                    warn!("LmStudioReranking: falling back to original retrieval scores");
                    results.iter().map(|r| r.score()).collect()
                })
            }
            Err(e) => {
                warn!("LmStudioReranking: client error: {e}. Falling back to original scores.");
                results.iter().map(|r| r.score()).collect()
            }
        };
